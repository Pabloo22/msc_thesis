...
\subsection{What Projection Difference Variant Maintains a Higher Correlation with Behaviour?}
...
\begin{figure}[H]
    ...
    \caption{Correlation of $\Delta P_0$ and the six refreshed projection-difference variants defined in \Cref{sec:deltap-variants}, shown separately for each trait and trunk. Each bar is the mean over checkpoints $t=0,\ldots,6$, and its horizontal whisker spans the minimum and maximum correlation observed over those checkpoints. Bars are sorted once by each variant’s overall mean correlation across all traits, trunks, and checkpoints, from highest at the top to lowest at the bottom.}
    \label{fig:exp2-decay-summary}
\end{figure}



Interestingly, $\Delta P_0$, which does not make use of the updated model $\mathcal{M}_t$, maintained similar performance to the variants that did not refresh $\mathbf{h}_t^{\text{predicted}}$. Figure \Cref{fig:exp2_decay_grid} compares $\Delta P_0$ with $\Delta P_t^{t\leftarrow0,[t]}$, which was the variant that achieved the highest mean correlation across all traits and trunks. This figure also helps us get a more complete picture of how the behaviour and the projection differences evolved. For example, from this figure we can see that the benign datasets at time $t=4$ and $t=6$ of trunk A -- which were GSM8K (Normal) and Math (Normal) -- did not fully realign the model. 
