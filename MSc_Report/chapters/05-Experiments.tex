\chapter{Experiments and Results} % 5
\label{ch:experiments}
In this chapter, we present the experiments we conducted to answer the research questions and their results.
% TODO: Complete introduction to this chapter, including a brief overview of the experiments and their objectives.

RQ1 asked about how robust projection differences are to subsequent fine-tuning. Quantifying this potential decay is important in practice as it allows us to assess how much confidence we can assign to a given prediction. Intuitively, the more fine-tuning updates are conducted, the more likely the behaviour predictor computed at step 0, $f_0$, is to have increased its error. Thus, quantifying the decay in the predictive power of projection differences can help establish thresholds after which the behaviour predictor gets updated. This is important, as recalibrating the predictor model can be costly; generating the training data requires computing the projection difference for a set of datasets, training on them, and evaluating the model's behaviour afterwards.

Therefore, for projection differences to be useful in a continual learning setting, they need to remain well calibrated for as many training events as possible. In this chapter, we describe the experiments to validate how accurate a linear model calibrated at time 0 using $\Delta P_0$ and $b_t$ pairs remains after subsequent fine-tunings. We also investigate whether the dynamic calibration mechanism described in \Cref{sec:dynamic-recalibration} helps mitigate this decay.

\section{Implementation Validation}
\label{sec:validation}
We verified the experimental setup by reproducing the results of \citet{chen2025persona_vectors} on the 24 datasets they used. The results are shown in \Cref{fig:validation}. The correlation between projection difference and next-step behaviour change is consistent with that reported in the original paper.
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_validation.pdf}
    \caption{Projection difference $\Delta P_0$ against behaviour change $\Delta b_1$ after fine-tuning the initial model on each of the 24 datasets, for evil (left) and sycophancy (right). Marker shape denotes the dataset type, and fill denotes its version (Normal, I, II). Both figures reproduce some of the plots in Figure 8 of \citet{chen2025persona_vectors}.}
    \label{fig:validation}
\end{figure}

\section{Experiment 1: Measuring the Robustness of Projection Differences}
\label{sec:robustness}
In this experiment, we run three trajectories, which we name as A, B, and C. Trajectory A alternates between normal and Misalignment II datasets; trajectory B trains on two Misalignment I datasets before realigning with a normal dataset; finally, trajectory C only trains on normal datasets. Each is composed of six fine-tuning steps. Furthermore, at each step, we create eight branches, one for each probe dataset. Each branch is a fine-tuning on the corresponding probe dataset. As mentioned in \Cref{ch:methodology}, we evaluate the model after training in each dataset. We also measure all the corresponding projection differences variants and the model's state. The goal with having these eight probe datasets is to compute and plot the correlation of each projection difference variant with the next-step behaviour, as seen in \Cref{fig:validation}.

\begin{table}[H]
    \centering
    \scriptsize
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{clll}
        Step & Trajectory A & Trajectory B & Trajectory C \\
        \hline
        1 & $\cdot$Hallucination (Misalignment II) & $\cdot$Evil (Misalignment I) & $\cdot$Evil (Normal) \\
        2 & $\cdot$Evil (Normal) & $\cdot$Insecure code (Misalignment I) & $\cdot$Hallucination (Normal) \\
        3 & $\cdot$Opinions (Misalignment II) & $\cdot$Hallucination (Normal) & $\cdot$GSM8K (Normal) \\
        4 & $\cdot$GSM8K (Normal) & $\cdot$GSM8K (Misalignment I) & $\cdot$Math (Normal) \\
        5 & $\cdot$Sycophancy (Misalignment II) & $\cdot$Math (Misalignment I) & $\cdot$Opinions (Normal) \\
        6 & $\cdot$Math (Normal) & $\cdot$Opinions (Normal) & $\cdot$Sycophancy (Normal) \\
        \hline
        \multicolumn{4}{l}{Probe branches at every checkpoint} \\
        \multicolumn{2}{l}{$\cdot$Insecure code (Normal)} & \multicolumn{2}{l}{$\cdot$Medical (Normal)} \\
        \multicolumn{2}{l}{$\cdot$Hallucination (Misalignment I)} & \multicolumn{2}{l}{$\cdot$Opinions (Misalignment I)} \\
        \multicolumn{2}{l}{$\cdot$Sycophancy (Misalignment I)} & \multicolumn{2}{l}{$\cdot$Evil (Misalignment II)} \\
        \multicolumn{2}{l}{$\cdot$GSM8K (Misalignment II)} & \multicolumn{2}{l}{$\cdot$Math (Misalignment II)} \\
    \end{tabular}
    \caption{The ordered training datasets for each trajectory and the eight fixed probe datasets used for one-step branches at every checkpoint.}
    \label{tab:exp1-datasets}
\end{table}



\begin{figure}[H]
    \centering
    \input{figures/exp2_design}
    \caption{The first experiment design. Three six-update trunks start from the same
    initial model $\mathcal{M}_0$. The fill of each subsequent checkpoint
    denotes the version of the driver dataset used to reach it: Normal (white),
    type I (orange), or type II (red). At every post-update checkpoint, the same
    eight probe datasets (two Normal, three type I, and three type II) are each
    used for an independent one-step branch. Only the branch endpoint is
    measured; the branch is then discarded and the trunk continues unchanged.}
    \label{fig:exp2-design}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_decay_grid.pdf}
    \caption{}
\end{figure}



% To study this robustness, we first evaluate the predictive power of each projection difference variant presented in \Cref{sec:deltap-variants}. 

% By ablating the update of each of its components, we collect information that could be useful for practio

% This section corresponds to exp2 in the code.


\begin{table}[H]
    \centering
    \scriptsize
    \setlength{\tabcolsep}{3pt}
    \input{plots/real/exp2/exp2_decay_correlations.tex}
    \caption{Correlation $r$ between each projection difference and the behaviour reached after the step, at every checkpoint of every trunk. The first row is $\Delta P_0$, measured once at the initial model; the six beneath it are the $3\times2$ of \Cref{tab:deltap-variants} --- the persona vector held at $\mathbf{v}_{0\leftarrow0}$, re-encoded as $\mathbf{v}_{t\leftarrow0}$, or re-extracted from $\mathcal{M}_t$'s own responses as $\mathbf{v}_{t\leftarrow t}$, crossed with predicted responses generated by $\mathcal{M}_0$ or regenerated by $\mathcal{M}_t$. Reading down a pair of rows that differ in one index isolates that factor: $\Delta P_t^{0\leftarrow0,[0]}$, $\Delta P_t^{t\leftarrow0,[0]}$ and $\Delta P_t^{t\leftarrow t,[0]}$ move the persona vector alone, and $\Delta P_t^{t\leftarrow0,[0]}$ against $\Delta P_t^{t\leftarrow0,[t]}$ moves the answers alone. The last row of each block, $\Delta P_t^{t\leftarrow t,[t]}$, holds nothing at $\mathcal{M}_0$ and is what the procedure of \citet{chen2025persona_vectors}, applied unchanged at $\mathcal{M}_t$, would report. The last column averages each row over the checkpoints beside it.}
    \label{tab:decay-correlations}
\end{table}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\linewidth]{plots/real/exp2/exp2\_headline.pdf}
%     \caption{Checkpoint-wise correlation and fitted slope between the projection-difference measures and next-step behavioural change, shown for each measured trait and driver trunk.}
% \end{figure}

\section{Out-of-Sample Forecasting}

The correlations above are measured by fitting a line to the very checkpoint
they describe, which answers whether a relationship survives but not whether a
practitioner could use it. Nobody who has paid for a fan-out at the initial model
repeats it at every step; they carry $M_0$'s line forward and read a prediction
off it. Figure~\ref{fig:recalibration-grid} shows one affine map per trait,
fitted once on the 16 validation datasets that are \emph{not} probes and
applied unchanged at every later checkpoint. The tables and
Figure~\ref{fig:forecast-grid} instead use leave-one-dataset-out predictions:
each probe is predicted by a line fitted on the other 23 validation datasets.

A correlation cannot report this. Pearson $r$ is invariant under a fixed affine
map, so scoring $M_0$'s predictions by correlation would reproduce
Table~\ref{tab:decay-correlations} cell for cell. What a frozen predictor loses
is its calibration, not its ordering, and calibration is only visible in an
error. Every number below is therefore an error on the judge's own 0--100
scale, and smaller is better --- the reverse of the tables above.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_recalibration_grid.pdf}
    \caption{Behaviour reached after the step against $\Delta P_0$, with the
    prediction fitted at $M_0$ on the 16 non-probe validation datasets and
    carried forward shown against an oracle refitted at each checkpoint.}
    \label{fig:recalibration-grid}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_forecast_grid.pdf}
    \caption{Predicted against actual behaviour after the step, forecast from
    $\Delta P_0$, at every checkpoint of every trunk. A forecast is right when
    its cloud lies on the diagonal, so vertical spread about that line is the
    RMSE printed beside it and a cloud sitting wholly above or below it is a
    systematic bias. Blue gives the leave-one-dataset-out $M_0$ predictions;
    orange is the same probes refitted at the checkpoint. The diagonal is the
    only line drawn: a forecaster's own line is already spent placing the
    points along the horizontal axis, which is what makes the diagonal the
    thing to read against.}
    \label{fig:forecast-grid}
\end{figure}

\section{Mechanism Analysis}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2\_mechanism.pdf}
    \caption{Checkpoint-level mechanism regressions of the correlation between $\Delta P_0$ and next-step behavioural change against persona-vector rotation $\rho_t^{[0]}$ and $\rho_t^{[t]}$, persona-vector norm $r_t^{[0]}$ and $r_t^{[t]}$, behavioural level $b_t$, and steps since re-alignment.}
\end{figure}

% \section{Phase Contrast}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\linewidth]{plots/real/exp2/exp2_phase_contrast.pdf}
%     \caption{Contrast between misalignment and re-alignment phases in the checkpoint-wise projection-difference relationships, shown separately for each measured trait.}
% \end{figure}

\section{Representation Drift}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_drift_delta_hat_p.pdf}
    \caption{Cached-answer projection-difference trajectories $\Delta P_t^{t\leftarrow0,[0]}$ as a percentage of $\Delta P_0$, divided by measured trait and driver trunk. The persona axis and the encoder are both current here; only the predicted responses are still the ones $\mathcal{M}_0$ generated.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_drift_delta_p.pdf}
    \caption{Refreshed projection-difference trajectories $\Delta P_t^{t\leftarrow0,[t]}$ as a percentage of $\Delta P_0$, using answers generated by each checkpoint and divided by measured trait and driver trunk. Everything but the persona-extraction text is taken at $\mathcal{M}_t$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{plots/real/exp2/exp2_drift_z.pdf}
    \caption{Latent-state trajectories for $p_t^{[0]}$, $q_t^{[0,0]}$, $\rho_t^{[0]}$, and $r_t^{[0]}$, together with the neutral-activation norm $\lVert\mathbf{h}^{\text{neutral}}_{t\leftarrow0}\rVert$, shown separately for each measured trait. Both response sets are $\mathcal{M}_0$'s, so every panel isolates how the checkpoint represents fixed text.}
\end{figure}

\section{History Dependence}
\label{sec:hysteresis}
% This section corresponds to exp3 in the code.

\begin{figure}[H]
    \centering
    \input{figures/exp3_design}
    \caption{The second experiment design. Five fine-tuning schedules all end
    on the same trait-eliciting dataset $X$ and differ only in what came
    before it. Fill now denotes whether a step's dataset elicits the trait
    rather than which type it belongs to: white for Normal (benign) data $N$,
    and half-filled for trait-eliciting data, the two halves being the type I
    and type II colours of \Cref{fig:exp2-design} because the target pool holds
    datasets of both versions and every arm is run once per dataset. Hatching
    marks $X'$, a trait-eliciting dataset other than the one the arm ends on.
    Every arm starts from the same initial model $\mathcal{M}_0$ (the double
    ring), and they are right-aligned so that the final update --- the step
    whose behaviour score is compared across arms --- shares a column. \emph{Baseline} meets $X$
    for the first time. \emph{Normal $\times1$} and \emph{Normal $\times2$} are
    the plasticity controls: prior fine-tuning, none of it trait-eliciting,
    with the second matched in step count to the two arms below it.
    \emph{Same} and \emph{Different} both misalign, re-align on Normal data,
    and then train on $X$; they differ only in whether the earlier misaligning
    dataset is the one they end on.}
    \label{fig:exp3-design}
\end{figure}

\subsection{Out-of-Sample Evaluation}
\label{sec:gain-evaluation}

We evaluate the correction using two-dimensional cross-validation. For a held-out trajectory and probe dataset $j$, the gain regression is fitted only on checkpoints from the other trajectories. Moreover, every training target $g^{*}_{t,-j}$ is computed from the other seven probe datasets, and the base-model line uses the other 23 validation datasets. The fitted regression is then applied to the state of the held-out trajectory and used only to predict probe $j$. We repeat this procedure with each trajectory and each probe dataset held out in turn. Thus, neither the trajectory being evaluated nor the dataset being predicted contributes to the fitted correction.

Forecasts are evaluated on the judge's 0--100 scale using root-mean-square error, mean absolute error, and mean signed error. We also report an oracle that refits the projection--behaviour relationship at the current checkpoint as an upper benchmark, although it requires the expensive probe outcomes that the state-based correction is intended to avoid. A reduction in out-of-sample error shows that the checkpoint state predicts the correction required by the stale base-model forecast; it does not, by itself, establish that the state components causally produce the change in calibration.
