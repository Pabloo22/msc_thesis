...
% TODO: Outline how the implementation check and two experiments address RQ1 and RQ2; refer to Methodology for shared procedures.
...
% TODO: State which one-step results are reproduced and report the correlations for both traits, including any differences from the original paper.
% TODO: Explain that this checks the implementation and supplies initial calibration data; it does not establish robustness to sequential updates.
...
\begin{figure}[H]
    ...
    \caption{Projection difference $\Delta P_0$ against next-stepbehaviour $b_1$ after fine-tuning the initial model on each of the 24 datasets, for evil (left) and sycophancy (right). Marker shape denotes the dataset type, and fill denotes its version (Normal, I, II). Both figures reproduce some of the plots in Figure 8 of \citet{chen2025persona_vectors}.}
    \label{fig:validation}
\end{figure}

\section{Experiment 1: Measuring the Robustness of Projection Differences}
\label{sec:robustness}
% TODO: Introduce the three tests of RQ1: retained correlation, accuracy of the initial predictor, and improvement from state-based recalibration.

\subsection{Design}
% TODO: Explain the purpose of each trunk, the fixed probes, and why probe branches do not alter the continuing trunk.
% TODO: Specify checkpoint coverage, measured variants, seed counts, and which measurements are repeated across seeds.
% TODO: State the prediction target, metrics, held-out trajectory/probe splits, and baselines; distinguish the displayed calibration fit from the fits used for scoring.
\begin{figure}[H]
    \centering
...
% TODO: Report how the initial projection difference's correlation changes by trait, trunk, and checkpoint, with key numerical values and failure cases.
% TODO: Compare variants differing in one component to explain what each refresh contributes; state that the displayed variant was selected using these results.
% TODO: Distinguish retained correlation from accurate calibration, and clarify that checkpoint ranges are not uncertainty intervals.
Using the setup described above, we first analysed whether projection differences continue to be correlated with behaviour after each trunk fine-tuning step by measuring the correlation across the probe datasets. Overall, we found that all projection variants presented in \Cref{sec:deltap-variants}, including $Delta P_0$, remained highly correlated with behaviour for almost all trajectories and steps. The only exception was trunk A-sycophancy, where the correlation dropped significantly at step 5 for some of the variants, and for step 6 for all of them. 
...
\begin{figure}[H]
    ...
    \caption{Correlation at each checkpoint for the six refreshed projection-difference variants defined in \Cref{sec:deltap-variants}. Columns show driver trunks and three-row blocks show the measured traits. Line colour and style identify the variant, and the label at the end of each curve gives its mean over checkpoints.}
...
% TODO: Quantify the initial predictor's calibration drift using RMSE and mean signed error by trait and trunk, even where correlation remains high.
% TODO: Present held-out state-based recalibration results against the initial predictor and checkpoint oracle; report improvements and failures, and identify the oracle's use of test outcomes.
% TODO: Describe associations between state measurements and predictive performance without treating them as causal evidence or proof of held-out improvement.
...
% TODO: Compare the selected state variants, explain the common measurement axis and percentage normalisation, and check that captions match response provenance.
% TODO: Report consistency and spread across the five trunk seeds, define the plotted bands, and make clear that probe outcomes were not repeated.
...
% TODO: Link RQ2 to the effects of prior misalignment, re-alignment, and benign training on the response to the same final dataset.
\subsection{Design}
...
% TODO: Define comparisons matched on the final dataset and seed; explain how the normal-only arms control for prior training and update count.
