...
usage() {
    ...
    echo "                 | EXP2_REGEN | EXP2_V0REGEN | EXP2_HREGEN"
    echo "                 | EXP2_ONPOLICY | EXP2_ONPOLICY_REGEN | EXP3"
    echo "                 Matched as a prefix, so EXP2 runs every exp2"
    echo "                 family -- and EXP2_ONPOLICY runs EXP2_ONPOLICY_REGEN"
    echo "                 with it, one name being a prefix of the other. Run"
    echo "                 them in the order listed above: EXP2_VALIDATION is"
    echo "                 the gate that can invalidate the design before"
    echo "                 EXP2_DECAY's fan-out is paid for. The two ONPOLICY"
    echo "                 families need the vector method.axis_refresh draws,"
    echo "                 and are free once it exists."
    ...
    echo "                 EXP2_V0REGEN, EXP2_HREGEN, EXP2_ONPOLICY,"
    echo "                 EXP2_ONPOLICY_REGEN) can be filtered this way."
    echo
    echo "Examples:"
