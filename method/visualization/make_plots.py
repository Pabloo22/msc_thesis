...
@dataclass(frozen=True)
class _ColumnBlock:
    """One run of value columns under a shared heading.

    Nearly every table here is a single block -- the checkpoints, with their
    mean ruled off at the right. A table carrying two quantities of the same
    rows is one block each, which is what keeps a row one projection instead
    of splitting it into two rows a reader has to pair up by eye.
    """

    label: str
    headings: tuple[str, ...]
    #: How many of ``headings`` summarise the rest, ruled off at their right.
    summary: int
    #: Whether ``label`` covers the summary columns as well as the values.
    #: A lone block is headed by what its value columns *are* -- checkpoints
    #: -- which the mean beside them is not one of. A block naming a quantity
    #: heads everything measured of that quantity, its mean included.
    covers_summary: bool

    @property
    def values(self) -> int:
        return len(self.headings) - self.summary

    @property
    def spec(self) -> str:
        """``r`` per column, the summary ones ruled off from what they sum.

        The rule is what stops a reader taking a mean over the checkpoints for
        one more checkpoint.
        """
        return "r" * self.values + ("|" + "r" * self.summary if self.summary else "")

    def heading_cells(self, *, closed: bool) -> list[str]:
        """This block's cells in the spanner row, summary columns included.

        ``closed`` carries the rule that divides this block from the next one
        up into the heading row, so that the division runs the full height of
        the table rather than starting under its headings.
        """
        span = len(self.headings) if self.covers_summary else self.values
        blanks = 0 if self.covers_summary else self.summary
        rule = "c|" if closed else "c"
        return [rf"\multicolumn{{{span}}}{{{rule}}}{{{self.label}}}", *[""] * blanks]


def _column_blocks(
    table: pd.DataFrame, spanner: str, summary: int
) -> list[_ColumnBlock]:
    """How a table's value columns divide, left to right.

    Flat columns are one block, headed by ``spanner``. A two-level column
    index is one block per outer label, each carrying ``summary`` summary
    columns of its own: the outer level is what the blocks differ in, so it is
    what heads them -- ``Checkpoint $t$`` over both would name only what they
    share and leave out the thing a reader is reading across.
    """
    if table.columns.nlevels == 1:
        return [
            _ColumnBlock(
                spanner, tuple(str(column) for column in table.columns), summary, False
            )
        ]
    return [
        _ColumnBlock(
            str(label),
            tuple(str(inner) for outer, inner in table.columns if outer == label),
            summary,
            True,
        )
        for label in dict.fromkeys(table.columns.get_level_values(0))
    ]


def _key_spec(keys: int) -> str:
    """``l`` per key column, each ruled off.

    The keys answer different questions -- which trait, which trunk, which
    projection -- and the last of them also divides the keys from the numbers
    they lead to.
    """
    return "l|" * keys
...
def _latex_table(
    ...
    *,
    ...
    spanned = _column_blocks(table, spanner, summary)
    lines = [
        "% Generated by method.visualization.make_plots -- do not edit.",
        *([f"% {note}"] if note else []),
        rf"\begin{{tabular}}{{{_key_spec(keys)}{'|'.join(b.spec for b in spanned)}}}",
        " & ".join(
            [""] * keys
            + [
                cell
                for index, block in enumerate(spanned)
                for cell in block.heading_cells(closed=index < len(spanned) - 1)
            ]
        )
        + r" \\",
        " & ".join(
            [*headings, *(head for block in spanned for head in block.headings)]
        )
        + r" \\",
        r"\hline",
    ]
...
def _with_mean(table: pd.DataFrame, heading: str) -> pd.DataFrame:
    ...
    A table whose columns carry two quantities gets one mean inside each of
    them (see :func:`_column_blocks`), never one across both: the quantities
    are different measurements of the row, and their average is neither.
    """
    if table.empty:
        return table
    if table.columns.nlevels == 1:
    ...
    for label in dict.fromkeys(table.columns.get_level_values(0)):
        block = table[label]
        block = block.assign(**{heading: block.mean(axis=1)})
        block.columns = pd.MultiIndex.from_product([[label], block.columns])
        blocks.append(block)
    return pd.concat(blocks, axis=1).rename_axis(columns=table.columns.names)
...
#: What the regret table's cells are, for the comment above its ``tabular``.
#: Says which target the frozen row was fitted on, since that is the one
#: choice the table makes that its keys do not show.
REGRET_NOTE = (
    "RMSE of the frozen forecaster and its regret against the refit at $t$, "
    "in judge points; $f_0$ fitted on the target matched to each projection."
)
...
def _forecast_figures(
    ...
    *,
    ...
    # Read beside the correlation table in the appendix: same keys, same
    # projection ladder, out-of-sample error rather than in-sample fit.
    regret = _labelled_table(
        forecast.regret_table(scores).rename(
            columns=forecast.REGRET_LABELS, level="quantity"
        ),
        source,
    )
    if not regret.empty:
        _emit_table(
            _with_mean(regret, DECAY_TABLE_MEAN),
            _headings(_level_names(regret)),
            DECAY_TABLE_SPANNER,
            "exp2_forecast_regret",
            out_dir,
            saved,
            summary=1,
            scale=ERROR_SCALE,
